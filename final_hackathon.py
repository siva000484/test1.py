...
new_transcriptions = ["Customer: I'm sorry upset about my overdraft fees. Colleague: Let's review your account and see what we can do.", 
                      ...
                      "I'm angry to hear that you are not having any issues with your account. glad to hear that.",
...
df1 = pd.DataFrame({'Name of the Customer': [], 'Contact No:': [], 'Issue:': [], 'Emotion severity:': [],'Complaint status:': []})
# print(df1)
...
for i in range(len(predictions)):
    ...
    # df1['Issue:'] = ['new_transcriptions[i]']
    print(f"Predicted severity: {predicted_label}\n")

    df1.loc[i, 'Issue:'] = new_transcriptions[i]
    df1.loc[i, 'Emotion severity:'] = predicted_label
    if predicted_label == 'Happy':
        df1.loc[i, 'Complaint status:'] = 'Not a Complaint'
    else:
        df1.loc[i, 'Complaint status:'] = 'Complaint'    

     
df1.fillna({'Name of the Customer': '**********', 'Contact No:': '0000000000'}, inplace=True)
